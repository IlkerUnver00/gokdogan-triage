...
def render_html(report: TriageReport) -> str:
    ...
    parts: list[str] = []
    parts.append(f"<title>gokdogan — {_esc(f.path)}</title><style>{_CSS}</style>")
    ...
    ident = [
        ...
        ("impfuzzy", f'<span class="mono">{_esc(f.impfuzzy or "-")}</span>'),
        ("rich_hash", f'<span class="mono">{_esc(report.rich.hash) if report.rich else "-"}</span>'),
        ("authentihash", f'<span class="mono">{_esc(f.authentihash or "-")}</span>'),
    ...
    parts.append('<div class="foot">Generated by gokdogan — static triage, no code executed. '
